...
if [[ "${BRANCH}" =~ ^(flippy)$ ]]; then
  ...
  sed -i '0,/case \$BRANCH in/{
	...
	flippy)\
		...
		declare -g -i KERNEL_GIT_CACHE_TTL=120 # 2 minutes; this is a high-traffic repo\
		KERNELSOURCE='"'"'https://github.com/ramir-o/linux-6.18.y.git'"'"'\
		KERNELBRANCH='"'"'branch:main'"'"'\
		;;
...
cp -f ${GITHUB_WORKSPACE}/patch/A5E/sunxi-dram-a523-alternative-parameters.patch patch/u-boot/v2026.01/board_radxa-cubie-a5e/
